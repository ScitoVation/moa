function model = model_skn(X,class,settings);

% Supervised Kohonen Network
% model_skn builds Supervised Kohonen Network
% 
% model = model_skn(X,class,scaler,settings);
% 
% input:
...
gXsca=[Xsca,settings.scalar.*in_out];

% calculates the map
[net,count_epo] = som_net(gXsca,settings,class);
