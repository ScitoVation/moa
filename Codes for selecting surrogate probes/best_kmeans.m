...
N=500;
Kpool=1:N:dim(1);
distortion=zeros(length(Kpool),1);
for k_temp=1:length(Kpool)
    k_temp
    [~,~,sumd]=kmeans(X,Kpool(k_temp),'emptyaction','drop');
    destortion_temp=sum(sumd);
    % try differnet tests to find minimun disortion under k_temp clusters
    parfor test_count=2:test_num
        [~,~,sumd]=kmeans(X,Kpool(k_temp),'emptyaction','drop');
...
distortion_percent=cumsum(variance)/(distortion(1));
