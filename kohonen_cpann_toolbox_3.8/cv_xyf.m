function cv = cv_xyf(X,class,settings,cv_type,cv_groups)

% cross validation for X-Y fused networks (XYFs)
...
for i=1:length(canc_groups)
    ...
    % calculates model
    model = model_xyf(X_training,class_training,settings);
    if isstruct(model)
        pred = pred_xyf(X_test,model);
