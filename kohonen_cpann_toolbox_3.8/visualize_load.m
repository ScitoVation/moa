...
if length(handles.vars_listed) == 0
    ...
    if strcmp(handles.load_type,'class')
        errortype = check_classes(handles);
    elseif strcmp(handles.load_type,'model')
        errortype = check_model(handles);
    elseif strcmp(handles.load_type,'settings')
    ...
    elseif strcmp(handles.load_type,'optimisation')
        errortype = check_optimisation(handles);
    end
...
% -------------------------------------------------------------------------
function errortype = check_classes(handles)
errortype = 'none';
...
% class is string 
if isstr(class)                                     
    errortype = 'input error: class labels must be numbers (strings are not allowed)';
    return
end
% number of classes 
if max(class) < 2 
...
if ~isfield(model,'type') | ~isfield(model,'net') | ~isfield(model,'scal') | ~isfield(model,'res')
    errortype = 'input error: the selected structure is not recognized as a model created by this toolbox';
    return
end

% -------------------------------------------------------------------------
function errortype = check_optimisation(handles)
errortype = 'none';
if handles.output.from_file == 1
    tmp_data = load(handles.output.path);
    in = get(handles.listbox_variables,'Value');
...
% model is a toolbox structure 
if ~isfield(model,'chrom') | ~isfield(model,'crit') | ~isfield(model,'settings')
    errortype = 'input error: the selected structure is not recognized as optimisation results created by this toolbox';
    return
end
...
if handles.output.from_file == 1
    tmp_data = load(handles.output.path);
    in = get(handles.listbox_variables,'Value');
    settings = getfield(tmp_data,handles.vars_listed(in).name);
else
    in = get(handles.listbox_variables,'Value');
    settings = evalin('base',handles.vars_listed(in).name);
end
% settings is a toolbox structure
if ~isfield(settings,'name') | ~isfield(settings,'nsize') | ~isfield(settings,'epochs') | ~isfield(settings,'bound') | ~isfield(settings,'init') | ~isfield(settings,'scaling') | ~isfield(settings,'a_max')  | ~isfield(settings,'a_min') 
    errortype = 'input error: the selected structure is not recognized as settings created by this toolbox';
end
